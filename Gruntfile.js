...
module.exports = function (grunt) {
  ...
  // set maintainability index lower for large data maps
  configs.complexity.generic.options.maintainability = 56;

  // prepare files & folders for grunt:plato & coverage
  var autoLoadFilesAndFolders = function (baseData, isFile) {
    var fs = require('fs');
    ...
    ['/lib/commands/marionette/', '/lib/commands/webdriver/'].forEach(function (folder) {
      fs.readdirSync(__dirname + folder).forEach(function (file) {
        var platoFolder = 'files/' + folder.substring(1).replace(/\//g, '_') + file.replace('.js', '_js');
        if (isFile === true) {
          baseData.push(platoFolder + '/report.history.json');
        } else {
          baseData.push('report/complexity/' + platoFolder);
        }
      });
    });
  ...
  grunt.initConfig({
    ...
    bump: configs.bump({
      pushTo: 'git@github.com:dalekjs/dalek-browser-firefox.git',
    ...
    preparePlato: {
      options: {
        folders: autoLoadFilesAndFolders([
          'coverage',
          'report',
          'report/coverage',
          'report/complexity',
          ...
          'report/complexity/files/lib',
          'report/complexity/files/lib/commands',
          'report/complexity/files/lib_commands_marionette',
          'report/complexity/files/lib_commands_webdriver',
          'report/complexity/files/lib_webdriver_js',
          'report/complexity/files/lib_marionette_js'
        ], false),
        files: autoLoadFilesAndFolders([
          'report.history.json',
          'files/index_js/report.history.json',
          'files/lib_webdriver_js/report.history.json',
          'files/lib_marionette_js/report.history.json'
        ], true)
      }
    },
